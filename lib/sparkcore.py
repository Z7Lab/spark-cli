...
_CONFIG = [
    ...
    {"key": "finetune_dir",    "default": "~/spark-finetune",                    "env": "SPARK_FINETUNE_DIR",       "type": "str",  "init": False, "help": "LLM fine-tune stack dir on the DGX (datasets, configs, outputs, run state) for spark finetune"},
    {"key": "finetune_base_model", "default": "unsloth/Qwen2.5-Coder-7B-Instruct-bnb-4bit", "env": "SPARK_FINETUNE_BASE_MODEL", "type": "str", "init": False, "help": "Default base for spark finetune (HF repo or local path) — overridable per run with --base. Default is an Unsloth bnb-4bit coder for QLoRA; see dgx-spark-model-research.md for vetted GB10 picks"},
    {"key": "unsloth_image",   "default": "",                                    "env": "SPARK_UNSLOTH_IMAGE",      "type": "str",  "init": False, "help": "Operator-provided Unsloth image for spark finetune — REQUIRED, unset by default (no usable upstream image for GB10/sm_121). Set it: spark config set unsloth_image <image[@sha256:…]> (build from templates/finetune/Dockerfile.reference, or use a published GB10 image). spark drives it, never builds it"},
